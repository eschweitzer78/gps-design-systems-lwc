...
export default class SfGpsDsCaOnFormTelephone extends SfGpsDsFormText {
  ...
  /**
   * Returns the autocomplete value for telephone input.
   * Uses OmniScript configuration if set, otherwise defaults to "tel".
   * @returns {string} Autocomplete attribute value
   */
  get computedTelAutocomplete() {
    // Use OmniScript configured value if set
    if (this._autocomplete) {
      return this._autocomplete;
    }
    // Default to "tel" for telephone inputs (WCAG/accessibility best practice)
    return "tel";
  }
