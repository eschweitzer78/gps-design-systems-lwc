    ...
    fs.readFile(p)
      .then(async (data) => {
        ...
        for (const directory in config) {
          ...
          let hasScssExt =
            directory.endsWith(".scss") || directory.endsWith(".scss*");
          ...
          let targetFile = `${targetDir}/${file}${hasCssExt ? "" : hasScssExt ? "" : ".scss"}`;

          try {
            let content = hasScssExt
              ? await readFile(targetFile)
              : genFile(targetFile).css;
            ...
            for (const replacement of config[directory]) {
              if (replacement.extension) {
                ...
                // special case if the object has the target attribute it's about setting the file
                // and not a replacement directive
            ...
            const pContent = hasScssExt
              ? content
              : await postcss([postcssCustomMedia()]).process(content);
            ...
            await fs.writeFile(
              ...
              "/* sldsValidatorIgnore */\n" +
                (hasScssExt ? pContent : pContent.css),
              (err) => {
                if (err) throw err;
              }
            );
          } catch (e) {
            console.debug("error processusing file", e);
