...
let reThematicBreak = /^(?:\*[ \t]*){3,}$|^(?:_[ \t]*){3,}$|^(?:-[ \t]*){3,}$/;

let reMaybeSpecial = /^[#`~*+_=<>0-9-]/;

let reNonSpace = /[^ \t\f\v\r\n]/;

let reBulletListMarker = /^[*+-]/;

let reOrderedListMarker = /^(\d{1,9})([.)])/;

let reATXHeadingMarker = /^#{1,6}(?:[ \t]+|$)/;

let reCodeFence = /^`{3,}(?!.*`)|^~{3,}/;

let reClosingCodeFence = /^(?:`{3,}|~{3,})(?= *$)/;

let reSetextHeadingLine = /^(?:=+|-+)[ \t]*$/;

let reLineEnding = /\r\n|\n|\r/;

// Returns true if string contains only space characters.
let isBlank = function (s) {
  return !reNonSpace.test(s);
};

let isSpaceOrTab = function (c) {
  return c === C_SPACE || c === C_TAB;
};

let peek = function (ln, pos) {
  if (pos < ln.length) {
...
let endsWithBlankLine = function (block) {
  while (block) {
...
let addChild = function (tag, offset) {
...
let parseListMarker = function (parser, container) {
...
let closeUnmatchedBlocks = function () {
  if (!this.allClosed) {
...
let advanceOffset = function (count, columns) {
...
let advanceNextNonspace = function () {
  this.offset = this.nextNonspace;
  this.column = this.nextNonspaceColumn;
  this.partiallyConsumedTab = false;
};

let findNextNonspace = function () {
  let currentLine = this.currentLine;
  let i = this.offset;
...
let incorporateLine = function (ln) {
...
let finalize = function (block, lineNumber) {
...
let processInlines = function (block) {
  let node, event, t;
