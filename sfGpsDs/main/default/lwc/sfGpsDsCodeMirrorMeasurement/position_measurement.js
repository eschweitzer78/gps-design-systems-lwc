...
import { updateLineForChanges } from "c/sfGpsDsCodeMirrorDisplay";
...
function pageScrollX(ldoc) {
  // Work around https://bugs.chromium.org/p/chromium/issues/detail?id=489206
  // which causes page_Offset and bounding client rects to use
  // different reference viewports and invalidate our calculations.
  if (chrome && android)
    return -(
      ldoc.body.getBoundingClientRect().left -
      parseInt(getComputedStyle(ldoc.body).marginLeft, 10)
    );
  return (
    ldoc.defaultView.pageXOffset ||
    (ldoc.documentElement || ldoc.body).scrollLeft
  );
}
function pageScrollY(ldoc) {
  if (chrome && android)
    return -(
      ldoc.body.getBoundingClientRect().top -
      parseInt(getComputedStyle(ldoc.body).marginTop, 10)
    );
  return (
    ldoc.defaultView.pageYOffset ||
    (ldoc.documentElement || ldoc.body).scrollTop
  );
}

function widgetTopHeight(lineObj) {
  let { widgets } = visualLine(lineObj),
    height = 0;
  if (widgets)
...
export function cursorCoords(
  ...
  function getBidi(lch, partPos, invert) {
    let part = order[partPos],
      right = part.level === 1;
    return get(invert ? lch - 1 : lch, right !== invert);
  }
