...
const getCategoriesWithCookies = (
...
const findMatchingCookies = (
  allCookies: string[], cookieName: string | RegExp) => {
 ...
 */
export const autoclearCookiesHelper = (
  isFirstConsent: boolean
) => {
  const state = globalObj._state;
...
export const setCookie = (
  useRemainingExpirationTime?: boolean
) => {
 ...
 */
export const eraseCookiesHelper = (
  cookies: string[], 
  customPath: string, 
  customDomain: string
) => {
   ...
   */
  const erase = (cookie: string, domain: string) => {
 ...
 */
export const getSingleCookie = (name: string, getValue: boolean = false): string => {
 ...
 */
export const getAllCookies = (regex?: RegExp): string[] => {
