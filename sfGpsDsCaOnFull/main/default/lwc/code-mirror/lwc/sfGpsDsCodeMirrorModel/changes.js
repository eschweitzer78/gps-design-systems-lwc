...
import {
  ...
  regChange,
  regLineChange
} from "c/sfGpsDsCodeMirrorDisplay";
import { estimateHeight } from "c/sfGpsDsCodeMirrorMeasurement";
import {
  hasHandler,
  signal,
...
export function makeChange(doc, change, ignoreReadOnly) {
  if (doc.cm) {
    ...
    if (doc.cm.state.suppressEdits) return null; // ESC
  }
...
function makeChangeSingleDoc(doc, change, selAfter, spans) {
  ...
  if (change.from.line > doc.lastLine()) return null; // ESC
