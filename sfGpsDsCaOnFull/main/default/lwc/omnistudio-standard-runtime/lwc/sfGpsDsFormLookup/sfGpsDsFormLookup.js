...
import { api } from "lwc";
import OmniscriptLookup from "c/sfGpsDsOsrtOmniscriptLookup";
...
const CLASS_NAME = "sfGpsDsFormLookup";
const DEBUG = false;
...
export default class extends SfGpsDsOmniHasValidationMixin(OmniscriptLookup) {
  ...
  get computedOptions() {
    /* There used to be an oddity when options are set (in prefill) but generateUniqueAttributes() hasn't been called.
       And then there is neither key nor id, which is an issue to render the template...
       TODO: assess if this can be removed as it looks like the issue may have been files.
    */
