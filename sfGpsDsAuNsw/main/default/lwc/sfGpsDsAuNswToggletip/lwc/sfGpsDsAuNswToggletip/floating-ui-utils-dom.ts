...
export function getNodeName(node: Node | Window): string {
  if (isNode(node)) {
    return ((node as Node).nodeName || "").toLowerCase();
  }
...
export function getDocumentElement(
  ...
  return (
    (isNode(node) ? (node as Node).ownerDocument : (node as Window).document) || window.document
  )?.documentElement;
}
...
export function isOverflowElement(
  element: Element
): boolean {
...
export function isLastTraversableNode(
  node: Node
): boolean {
...
export function getNearestOverflowAncestor(
  node: Node
): HTMLElement {
